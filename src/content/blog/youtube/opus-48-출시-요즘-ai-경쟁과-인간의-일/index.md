...
흥미로운 건 이 두 달이 혼자 떠도는 숫자가 아니라는 점이다. 노정석은 업계 여기저기서 같은 표현이 나온다고 짚는다. 드와케시 파텔과 라이너 포프 편을 공부할 때도, 드와케시 파텔은 "프론티어 모델은 두 달쯤 배포되다가 두 달마다 새 모델로 교체된다"는 패턴을 당연한 전제처럼 깔고 있었다. 샘 올트먼도 호주 행사 화상 대담에서 "두 달 만에 세상이 바뀌는데 회사 연간 계획이 필요할까" 하는 뉘앙스의 말을 했다고 한다.
...
논리는 단순하다. 만족스러운 결과가 한번 나오고 나면, 그다음에 원하는 건 당연히 그게 빨리 나오는 것이다. 정해진 품질이 정해진 시간 안에 나온다면, 사람들은 그게 오퍼스든 뭐든 최고급 모델인지를 따지지 않는다. 그냥 "빠르고 좋은 모델"이라는 개념이 생긴다. 구글이 이번에 플래시 모델을 메인으로 내세운 이유도 여기 있을 것이라고 노정석은 본다. 다만 이 전략은 플래시의 품질이 맡은 일에 충분할 때만 통한다. 품질이 그 선에 못 미치는 순간 사람들은 곧장 윗 티어 모델로 갈아탄다. 그래서 어디까지 싸고 빠른 모델로 내려가도 사용자가 안 떠나는지, 다들 그 경계선을 더듬는 중이다.
...
그런데 최승준이 보기에 이건 앤트로픽이 내놓은 단발성 제품이 아니라 하나의 흐름이다. 여러 곳이 제각기 같은 방향으로 가고 있다는 것이다. 그는 먼저 매니지드 에이전트를 다룬 "뇌와 손을 분리하기"라는 글에서 인용됐던 연구를 꺼낸다. 열쇠는 REPL이라는 도구다. REPL은 코드를 한 줄 치면 바로 실행해 결과를 돌려주는 도구로, 보통은 사람이 직접 치고 결과를 보고 다시 고쳐 치며 그 피드백 루프를 손으로 돌린다. 그 루프를 사람이 아니라 LLM이 코드로 돌리게 하고, 모델이 다루는 맥락(컨텍스트)마저 그 도구 안에 놓인 객체처럼 두자는 발상이다. 여기서 인용된 논문이 Recursive Language Models(RLM)로, LLM이 코드를 실행하고 그 안에서 또 다른 작은 LLM을 재귀적으로 불러 쓰는 구조다.

![arXiv 'Recursive Language Models' 논문 화면](./152_860.jpg)

RLM 저자 알렉스 장은 "우리 페이퍼를 읽어봐라, 이미 다뤄둔 흐름이다"는 식으로 반응했는데, 이런 반응이 그 하나만은 아니었다. 그리고 이 RLM이 DSPy라는 도구 안에 들어가 있더라는 게 최승준의 관찰이다. RLM 논문 저자 중 오마르 카타브가 스탠퍼드에서 DSPy를 만든 사람이니 당연한 일이다.

![알렉스 장 트윗 + RLM 논문 abstract](./158_900.jpg)

여기서 최승준은 한 겹 위로 올라간다. DSPy든 텍스트그라드든 RLM이든, 결국 다 같은 일을 하고 있다는 것이다. 그는 이걸 메타 옵티마이제이션이라 부른다. 무언가의 성능을 끌어올리는 장치를 만들고, 그 장치를 다시 끌어올리는 장치를 그 위에 또 얹고, 그렇게 최적화의 층을 한 칸씩 쌓아 올리는 일이다. DSPy도 결국 프롬프트 자체의 성능을 그런 식으로 계속 끌어올릴 수 있다는 발상이고, 이건 카파시가 말하던 오토 리서치 개념과 똑같다.

그가 짚는 핵심은 이 한 문장에 모인다. **잘됐는지 점수를 매길 잣대(Evaluation Metric) 하나만 있으면, 모델은 우리가 무엇을 시키든 그냥 컴퓨팅을 쏟아부어 그 점수를 올리는 방향으로, 즉 답을 검색하듯 뒤지는 방향으로 풀어낸다.** 잣대만 주면 나머지는 계산량으로 밀어붙인다는 것이다. 그 위에서 보면 앞서 나온 것들이 다 한 줄로 꿰인다. RLM은 LLM이 코드를 실행하며 그 안에서 작은 LLM을 재귀적으로 부리는 이야기고, 매니지드 에이전트는 작업 상태를 모델의 머릿속(컨텍스트)이 아니라 바깥 객체로 빼두고 샌드박스와 REPL로 다루는 이야기고, 다이내믹 워크플로우는 그 조율을 컨텍스트가 아니라 스크립트(코드)에 두는 이야기다. 부르는 이름만 다를 뿐, 결국 코드로 짠 하네스가 모델을 꽉 붙들어 통제한다는 같은 그림이다. 하네스란 모델이 제멋대로 새지 않게 조이는 결정적인 통제 장치를 말한다.

바룬 모한의 그 빠르게 돌아가던 영상도 같은 층의 이야기일 것이라고 그는 추측한다. 모델이 혼자 다 해낸 게 아니라, 하네스로 짠 발판(스캐폴드)이 OS를 만들고 둠을 띄우는 긴 작업을 한 번에 쭉 밀고 간 것이다.

다만 그 긴 작업도 안을 들여다보면 깔끔한 한 방이 아니다. 수많은 에러를 끊임없이 점검하며 "이거 틀렸네 고치고, 또 틀렸네 고치고"를 반복하는, 끝없는 손질(틴커링)의 연속이다. 수학에서 벌어진 일도 비슷했다고 최승준은 말한다. 가설을 세우고, 실험하고, 문제가 생기면 기록하고, 그 기록에서 통찰을 얻어 다음 가설을 세우고, 이걸 계속 밀어붙이는 것.

이 흐름에 노정석은 코르카 CTO 강규영에게서 들은 표현을 얹는다. "코드 애즈 하네스(code as harness)", 코드가 곧 통제 장치가 된다는 말이다. 귀에 착 감기는 표현이었다고 한다. 강규영은 그러면서 클라우드플레어의 다이내믹 워크플로우와 프로젝트 씽크를 소개했는데, 여기서도 이름이 같다. 차세대 에이전트를 짤 때 코드가 하네스가 되고, 무엇을 가져오든 그걸 해낼 수 있는 플랫폼을 만든다는, 똑같은 이야기가 또 나온다. 그쪽에서 쓰는 말이 장기 실행 에이전트, 행위자 모델, 듀얼 오브젝트다.

![Cloudflare 'Project Think: 차세대 AI 에이전트 구축', 'code as harness' 실사례](./200_1100.jpg)

여러 회사가 같은 산을 저마다 다른 길로 오르고 있다. 자세히는 아니어도 느낌으로 그 방향이 잡힌다.
...
사람들은 과거의 기준으로 미래를 평가하는 나쁜 버릇이 있다. 과거의 패턴은 반복되지만, 그 패턴 속 세로축의 세기와 진폭은 물가를 보정하듯 상대적으로 조정해서 써야 한다. "예전 모바일에서 이만큼 갔으니 이번에도 그쯤이 피크"라는 말이 많이 틀릴 수도 있다고 그는 본다.

그가 존경하는 비노드 코슬라의 논리가 여기에 못을 박는다. 이건 기존 앱과 다르다, 왜냐하면 이건 지능이라서 천장이 없다는 것이다. 영원히 살 수 있는 약으로 암을 해결했다고 치자. 사람들이 거기서 멈출까. 아니다, 무한히 살고 싶을 거고, 그걸로도 안 끝나 더 예쁘고 건강한 삶을 찾을 거고, 그게 끝나면 또 다음을 찾는다. 인간의 욕구는 열려 있다. 과거 플랫폼 전환들은 내놓을 수 있는 천장이 꽉 닫혀 있었다. 인터넷이 나와도 TV나 신문 대비 몇 배, 모바일이 돼도 PC 못 사던 사람까지 더해 몇 배. 하지만 지능은 거의 영원히 수요가 증가한다. 노정석은 그 말에 깊이 동의하게 됐다고 한다.

가까운 미래로 내려오면, 오픈AI와 앤트로픽이 올해 안에 상장(IPO)을 할 수 있다는 뉴스가 많다. 상장을 하면 무슨 일이 생기느냐는 물음에 노정석은 답한다. 지금도 비상장 시장에서 자금 조달에 아무 문제가 없지만, 상장은 하나의 이정표다. 구성원들이 가진 주식의 유동성이 확 늘고, 비상장 투자에서 경영진을 옥죄던 많은 조건과 의무가 풀린다. 투자자들이 떠나고, 회사가 주도해 합법적으로 자금 조달을 이어가며 기업가치를 높일 수 있다. 그리고 기업가치는 일종의 순위표가 된다. 1등은 어디, 2등은 어디. 그 순위표를 그냥 목표로 찍고 가는 것 아니겠냐는 것이다. 투자자 입장에서 정작 중요한 질문은 따로 있다. 그 상장이 시작일 것이냐, 끝일 것이냐.

두 사람이 다음으로 들여다보려는 건 모델 위가 아니라 모델 아래다. 에이전트 시대가 되면서 모델이 실제로 답을 내놓는 추론 단계의 중요성이 어마어마하게 커지고, 토큰이 어마어마하게 밀려오고 있다. 그래서 그 "토큰 공장"의 안을 살펴보는 일에 의미가 있다고 본다. 하드웨어의 깊은 곳에서부터, 그것들을 조율하는 층, 그 위로 올라가는 소프트웨어 영역, 그 사이를 들여다보는 것. 모델이 두 달마다 갈아치워지는 속도의 한가운데서, 정작 집중하기로 한 건 그 속도를 떠받치는 바닥이다.
